...
set treeViews 0
if {![catch {package require BLT}]} {set treeViews 1}
...
proc magic::makecellmanager { mgrpath } {
   ...
   if {$treeViews} {
      ...
      blt::hiertable ${mgrpath}.box.view -bg white -allowduplicates false \
		-hideroot true -yscrollcommand "${mgrpath}.box.vert set" \
...
proc magic::maketechmanager { mgrpath } {
   ...
   wm protocol ${mgrpath} WM_DELETE_WINDOW "set Opts(techmgr) 0; \
		wm withdraw ${mgrpath}"
}
...
magic::tag load "magic::cellmanager; magic::captions"
...
if {$treeViews} {
   ...
   magic::tag cellname "magic::captions %1"
}
...
proc magic::mgrupdate {win {cmdstr ""}} {
   ...
   if {${cmdstr} == "read"} {
      if {$treeViews} { magic::cellmanager }
      magic::captions
      magic::techmanager update
...
proc magic::openwrapper {{cell ""} {framename ""}} {
   ...
   # Update the cell manager (if it exists)
